...
// Creo la funzione per cambiare la data nel formato europeo
function getEuDate(aDate) {

    const dateSplitted = aDate.split("-");

    return `${dateSplitted[1]}/${dateSplitted[0]}/${dateSplitted[2]}`

}

// Creo la funzione per trovare le iniziali 
function getInitialLetters(aString) {
    
    const stringSplitted = aString.split(" ");
